...
variables by analogy with PAM modules. So,
`SYSTEMD_EXEC_PID`[^systemd-exec-pid] environment variable stores `PID` process
which calls NSS service. When `PID` is known it enables to
get service name via `/proc/PID/comm`[^proc-pid-comm]. Thus, implementation of
checking of service name partly solves the problem and enables to use the tools
without adding users to a data source. Unlike the `nss-devel` in the `pam-devel`
library is an implemented function for getting a service name.
