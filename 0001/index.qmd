...
label:
  ...
  - break-glass
link:
  - "[#2](https://github.com/efrikin/rfds/pull/2){target=_blank_}"
  - "[#3](https://github.com/efrikin/rfds/pull/3){target=_blank_}"
  - "[#4](https://github.com/efrikin/rfds/pull/4){target=_blank_}"
  - "[#5](https://github.com/efrikin/rfds/pull/5){target=_blank_}"
  - "[#6](https://github.com/efrikin/rfds/pull/6){target=_blank_}"
  - "[#7](https://github.com/efrikin/rfds/pull/7){target=_blank_}"
open-graph: true
filters:
  - d2
d2:
...
```{.d2 file=diagrams/01-ssh-stack-protocols.d2 width=0%}
```

![SSH stack protocol](diagrams/rendered/diagram-1.svg){fig-caption="SSH stack protocol" fig-align="center" width=70% #fig-ssh-stack}
...
```{.d2 file="diagrams/08-sshd-nss.d2" width=0%}
```

![Conversation between sshd and NSS](diagrams/rendered/diagram-2.svg){fig-caption="Conversation between sshd and NSS" fig-align="center" width=80% #fig-sshd-nss}
...
```{.d2 file="diagrams/02-ssh-auth.d2" width=0%}
```
![SSH User Authentication protocol](diagrams/rendered/diagram-3.svg){fig-caption="SSH User Authentication protocol" fig-align="center" width=70% #fig-ssh-auth}
...
```{.d2 file="diagrams/03-ssh-auth-cert-1.d2" width=0%}
```

![SSH Authentication by Certificate](diagrams/rendered/diagram-4.svg){fig-caption="SSH Authentication by Certificate" fig-align="center" width=70% #fig-ssh-auth-cert-1}
...
```{.d2 file="diagrams/04-ssh-auth-cert-2.d2" width="0%"}
```

![SSH Authentication by Certificate](diagrams/rendered/diagram-5.svg){fig-caption="SSH Authentication by Certificate" fig-align="center" width=50% #fig-ssh-auth-cert-2}
...
them is `SSH_AUTH_INFO_0`.[^ssh-auth-info] It exposes authentication
information to PAM module (e.g. pubkey, certificate, etc.). This variable can
be used as source for making decisions during authorization process (e.g.
assigning sudo group to user).
...
```{.d2 file="diagrams/07-sshd-nss-pam.d2" width=0%}
```

![Conversation between sshd, NSS and PAM](diagrams/rendered/diagram-6.svg){fig-caption="Conversation between sshd, NSS and PAM" fig-align="center" #fig-sshd-nss-pam}
...
```{.d2 file="diagrams/05-ssh-nss-pam.d2" width=0%}
```

![Using NSS and PAM by SSH server](diagrams/rendered/diagram-7.svg){fig-caption="Using NSS and PAM by SSH server" fig-align="center" width=70% #fig-ssh-nss-pam }

### HLD

```{.d2 file="diagrams/09-hld.d2" width=0%}
```

![High Level Design](diagrams/rendered/diagram-8.svg){fig-align="center" #bg-hld}
...
[OpenSSH >= `7.6p1`](https://www.openssh.com/txt/release-7.6){target=_blank_}
...
```{.d2 file="diagrams/06-ssh-nss-pam-custom.d2" width=0%}
```

![Using custom NSS and PAM by SSH server](diagrams/rendered/diagram-9.svg){fig-caption="Using custom NSS and PAM by SSH server" fig-align="center" width=70% #fig-ssh-nss-pam-custom}
...
- [wh0: The SSH Protocol](https://wh0.github.io/2018/02/27/ssh-protocol.html){target=_blank_}
- [Teleport: SSH Certificates Security](https://goteleport.com/blog/ssh-certificates/){target=_blank_}
- [Using certificates for SSH authentication](https://lwn.net/Articles/913971/){target=_blank_}
- [Netburner: Introduction to the SSH Protocol](https://www.netburner.com/learn/introduction-to-the-ssh-protocol/){target=_blank_}
- [SecureW2: How Does SSH Certificate Authentication Work?](https://www.securew2.com/blog/how-does-ssh-certificate-authentication-work){target=_blank_}
- [NISTIR 7966: Security of Interactive and Automated Access Management Using SSH](https://nvlpubs.nist.gov/nistpubs/ir/2015/nist.ir.7966.pdf){target=_blank_}
- [Cloudflare: Fearless SSH: short-lived certificates bring Zero Trust to infrastructure](https://blog.cloudflare.com/intro-access-for-infrastructure-ssh/){target=_blank_}
